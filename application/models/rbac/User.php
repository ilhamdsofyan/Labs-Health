...
class User extends MY_Model {
	...
	public $datatable_columns = ['sys_user.id', 'username', 'email', 'status', 'sys_user.created_at'];
	...
	protected function buildDatatableQuery()
    {
        ...
        $this->db->join('sys_group_user', 'sys_group_user.user_id = sys_user.id', 'left');
        $this->db->group_start();
            $this->db->where_in('sys_group_user.group_id', $groups);
            $this->db->or_where(['sys_group_user.group_id' => null]);
        $this->db->group_end();

        $this->db->group_by('sys_user.username');

        if ($this->soft_delete === true) {
            $this->db->where(['sys_user.deleted_at' => null]);
        }
