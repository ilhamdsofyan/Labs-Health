 ...
 */
class MY_Orm extends CI_Model// implements \ArrayAccess
{
    /**
    ...
    /**
     * Indicates if the model should be blamed.
     *
     * @var bool
     */
    ...
    /**
     * Set false if your table dont't have deleted_at and deleted_by
     * @var boolean
     */
    ...
    /**
     * ORM dirty properties
     *
     * @var array
     */
    protected $_dirtyProperties = [];

    protected static $_schemaMapCache = [];
    ...
    function __construct()
    {
        ...
        /* Table Name Guessing */
        if (!$this->tableName) {
            $this->tableName = str_replace('_model', '', strtolower(get_called_class()));
        }
    }
    ...
    public function find($withAll=false)
    {
        ...
        $sqlFrom = ($instance->alias) ? "{$instance->tableName} AS {$instance->alias}" : $instance->tableName;
     ...
     *  $this->Model->findAll();
     */
    public function findAll($condition=[], $limit=null)
    {
        $instance = (isset($this)) ? $this : new static;

        if ($condition instanceof $instance->db) {
            $query = $condition;
        } else {
    ...
    public function insert($attributes, $runValidation=true)
    {
        ...
        $save = $this->_db->insert($this->tableName, $attributes);
    ...
    //     return $this->_db->insert_batch($this->tableName, $data);
    // }
    ...
    public function replace($attributes, $runValidation=true)
    {
        ...
        return $this->_db->replace($this->tableName, $attributes);
    }
    ...
    public function restore($condition=NULL)
    {
        ...
        if (static::SOFT_DELETED 
            ...
            return $query->update($this->tableName, $attributes);
    ...
    {
        $record = new static();
        $record->_readProperties  = $readProperties;
        $record->_selfCondition   = $selfCondition;
        $record->_dirtyProperties = [];
    ...
    public function save($runValidation=true)
    {
        ...
        if (!$this->_selfCondition) {
            ...
            if ($result) {
                ...
                $this->_selfCondition = $this->getLastInsertID();
                $this->_readProperties[$this->primaryKey] = $this->_selfCondition;
    ...
    public function getTableSchema()
    {
        ...
        if (!isset(self::$_ormCaches[$class])) {
            ...
            $columns = $this->_dbr->query("SHOW COLUMNS FROM `{$this->tableName}`;")
                ->result_array();
    ...
    protected function _attrEventBeforeInsert(&$attributes)
    {
        ...
        if ($this->blameable) {
            $this->_blameableBehavior(true, $attributes);
        }
    ...
    protected function _attrEventBeforeUpdate(&$attributes)
    {
        ...
        if ($this->blameable) {
            $this->_blameableBehavior(false, $attributes);
        }
    ...
    protected function _addSoftDeletedCondition()
    {
        ...
        elseif ($this->soft_delete === true) {
            // Add condition
            $this->_dbr->where([$this->_field('deleted_at') => null]);
        }
    ...
    {
        return ($this->alias) ? "`{$this->alias}`.`{$columnName}`" : "`{$this->tableName}`.`{$columnName}`";
    }
    ...
    public function __set($name, $value)
    {
        if ($this->propertyCheck) {
            $schema = $this->_getSchemaMap();
            if (!isset($schema[$name])) {
                throw new Exception("Property `{$name}` does not exist", 500);
            }
        }

        // kalau value sama, skip (important)
        if (
            array_key_exists($name, $this->_readProperties)
    ...
    public function __get($name)
    {
        ...
        // relationship lazy-load
        if (method_exists($this, $name)) {
            return $this->_getRelationshipProperty($name);
        }
    ...
    {
        return isset($this->_writeProperties[$name])
            || isset($this->_readProperties[$name])
    ...
    {
        unset(
            $this->_writeProperties[$name],
            $this->_readProperties[$name],
    ...
    {
        $dirty = [];
        foreach ($this->_dirtyProperties as $key => $_) {
            $dirty[$key] = $this->_writeProperties[$key] ?? null;
        }
        return $dirty;
    }
    ...
    {
        if ($field === null) {
            return !empty($this->_dirtyProperties);
        }
        return isset($this->_dirtyProperties[$field]);
    }
     ...
     * SCHEMA MAP (FAST)
     * ========================== */
    protected function _getSchemaMap()
    {
        $class = get_class($this);

        if (!isset(self::$_schemaMapCache[$class])) {
            $map = [];
            foreach ($this->getTableSchema() as $col) {
                $map[$col['Field']] = true;
            }
            self::$_schemaMapCache[$class] = $map;
        }

        return self::$_schemaMapCache[$class];
    }
     ...
     * @return [type]               [description]
     */
    private function _blameableBehavior($insert, &$attributes)
    {
        $blames = $this->blameableBehavior();

        if ($insert) {
            $field = $blames['createdByAttribute'];
        } else {
            $field = $blames['updatedByAttribute'];
        }

        if ($field !== null) {
            $attributes[$field] = $blames['value'];
        }
    ...
    {
        $instance = (isset($this)) ? $this : new static;
        $records = $record->get()->result_array();
